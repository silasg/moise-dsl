...
class IntegrationTest {
    ...
    def can_formulate_house_building_example = {
      ...
      // assert
      //assert(expectedXml == osXml, osXml.child diff expectedXml.child mkString(", "))
      new Comparison()(expectedXml, osXml) match {
        case NoDiff => assert(true)
        case Diff(position, msg) => assert(false, msg)
      }
    }
}
